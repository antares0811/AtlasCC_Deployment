if __name__ == '__main__':
    ...
    # nnUNetv2_predict -d 3 -f 0 -c 3d_lowres -i imagesTr -o imagesTr_predlowres --continue_prediction
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, '.checkpoints/STUNetTrainer_large_ft__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
        checkpoint_name='checkpoint_best.pth',
    )
    # variant 1: give input and output folders
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset003_Liver/infer'),
                                 join(nnUNet_raw, 'Dataset003_Liver/infer_res'),
                                 save_probabilities=False, overwrite=True,
                                 num_processes_preprocessing=2, num_processes_segmentation_export=2,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
    # # variant 2, use list of files as inputs. Note how we use nested lists!!!
    # indir = join(nnUNet_raw, 'Dataset003_Liver/imagesTr')
    # outdir = join(nnUNet_raw, 'Dataset003_Liver/imagesTr_predlowres')
    ...
    # img, props = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset003_Liver/imagesTr/liver_147_0000.nii.gz')])
    # img2, props2 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset003_Liver/imagesTr/liver_146_0000.nii.gz')])
    # img3, props3 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset003_Liver/imagesTr/liver_145_0000.nii.gz')])
    # img4, props4 = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset003_Liver/imagesTr/liver_144_0000.nii.gz')])
